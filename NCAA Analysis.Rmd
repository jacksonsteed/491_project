...
library("readxl")
  ...
  write_csv(testing_data, "TestData.csv" )
  write_csv(training_data, "TrainData.csv")
```
...
X_train <- training_data %>% select(pts, ast, drtg) %>% as.matrix()
y_train <- training_data$Ortg

X_test <- testing_data %>% select(pts, ast, drtg) %>% as.matrix()
