---
title: "R Notebook"
output: html_notebook
---

```{r}
#import data 
library("tidyverse") 
library("dplyr")
library("ggplot2")
library("readxl")

players2009 <- read_csv("Resources/CollegeBasketballPlayers2009-2021.csv")
players2022 <- read_csv("Resources/CollegeBasketballPlayers2022.csv")
drafted <- read_xlsx("Resources/DraftedPlayers2009-2021.xlsx")

selected_player_data <- players2009 %>% select(player_name, team, pid, ht, pts, ast, stl, blk, treb, Ortg, drtg, year, Min_per, TS_per, pick, twoPM, TPM, FTM)

summary(selected_player_data)
pts_box <- ggplot(selected_player_data, aes(y = pts), na.rm = TRUE) + geom_boxplot(fill = '#69b3a2', color='black', alpha = 1) + labs(title = "Boxplot of PTS")
pts_hist <- ggplot(selected_player_data, aes(x = pts)) + geom_histogram(binwidth = 1, fill = "yellow", color = "black") + labs(title = "Distribution of PTS", x = "PTS")
plot(selected_player_data[,"Ortg"])
pts_box
pts_hist
team <- selected_player_data[, "team"]
unique(team)
max(selected_player_data["Ortg"])
filter(selected_player_data, Ortg > 380)


```

```{r}
# Remove rows with NAs in the "pick" column
data_cleaned <- na.omit(selected_player_data, cols = "pick")
data_cleaned
plot(data_cleaned$Ortg, 
     type = "p")
plot(data_cleaned$drtg, 
     type = "p")
plot(data_cleaned$TS_per, 
     type = "p")
```


```{r}
install.packages(c("httr", "jsonlite", "dplyr"))
```

```{r}
# Does elevation change how a player performs?
# Similar players, abilities, check their elevation

library(httr)
library(jsonlite)
library(dplyr)

# Read the CSV file
cities <- read.csv("Resources/CityLatLong.csv")

# Create a function to get elevation
get_elevation <- function(lat, lon) {
  query <- paste0("https://api.open-elevation.com/api/v1/lookup?locations=", lat, ",", lon)
  res <- GET(query)
  data <- fromJSON(rawToChar(res$content))
  return(data$results$elevation)
}

# Add a new column 'elevation' to the dataframe
cities <- cities %>% 
  mutate(elevation = mapply(get_elevation, Lat, Long))

cities
```

